...
public class Zeit {
	...
	private void this_setOwner(Object owner, int key) {
		if (key == pmt.spielspaß.codegenerierung.ORMConstants.KEY_ZEIT_SPIELER) {
			this.Spieler = (pmt.spielspaß.codegenerierung.BugaBesucher) owner;
		}
		
		else if (key == pmt.spielspaß.codegenerierung.ORMConstants.KEY_ZEIT_BUGABESUCHER) {
	...
	public void setSpieler(pmt.spielspaß.codegenerierung.BugaBesucher value) {
		if (Spieler != null) {
			Spieler.leistung.remove(this);
		}
		if (value != null) {
			value.leistung.add(this);
		}
	}
